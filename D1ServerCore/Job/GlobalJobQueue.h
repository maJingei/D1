...
namespace D1
{
	...
	/**
	 * Flush 대기 중인 JobSerializer 의 전역 큐.
	 *
	...
	public:
		...
		/** JobSerializer 를 큐 끝에 추가하고 대기 Worker 하나를 깨운다. */
		void Push(std::shared_ptr<JobSerializer> Serializer);

		/**
		 * 큐에서 JobSerializer 하나를 꺼낸다.
		 * 큐가 비어있으면 Hybrid Wait (스핀 → WaitOnAddress) 로 대기한다.
		 *
		 * @param bShouldRun  false 가 되면 대기를 중단하고 nullptr 을 반환한다 (종료 신호).
		 */
		std::shared_ptr<JobSerializer> Pop(const std::atomic<bool>& bShouldRun);
	...
	private:
		...
		std::queue<std::shared_ptr<JobSerializer>> Serializers;
