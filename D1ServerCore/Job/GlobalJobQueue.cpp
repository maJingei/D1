...
namespace D1
{
	...
	{
		static GlobalJobQueue Instance;
		return Instance;
	}

	void GlobalJobQueue::Push(std::shared_ptr<JobSerializer> Serializer)
	{
		{
			std::lock_guard<std::mutex> Lock(Mutex);
			Serializers.push(std::move(Serializer));
	...
	std::shared_ptr<JobSerializer> GlobalJobQueue::Pop(const std::atomic<bool>& bShouldRun)
	{
		while (bShouldRun.load(std::memory_order_relaxed))
		{
			...
			for (uint32 SpinIdx = 0; SpinIdx < SPIN_COUNT; SpinIdx++)
			{
				{
					std::lock_guard<std::mutex> Lock(Mutex);
					if (Serializers.empty() == false)
					{
						std::shared_ptr<JobSerializer> Result = Serializers.front();
						Serializers.pop();
						return Result;
					}
				}
