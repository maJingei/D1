...
namespace D1
{
	...
	public:
		...
		/**
		 * 스프라이트 시트 텍스처와 프레임 크기를 설정한다.
		 * RenderSize는 InFrameSize와 동일 값으로 초기화된다 (1:1 출력).
		 *
		...
		/**
		 * 화면 출력 크기를 변경한다 (FrameSize와 다르면 확대/축소 출력).
		 * 픽셀 아트는 NearestNeighbor 보간으로 선명하게 확대된다.
		 */
		...
		/**
		 * 재생할 클립을 전환한다.
		 * 이미 같은 클립이 재생 중이면 무시하여 프레임이 리셋되지 않는다.
		 */
		void Play(int32 ClipId);
