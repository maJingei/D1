...
namespace D1
{
	...
	void Game::BeginPlay()
	{
		...
		// 6. 몬스터 액터 스폰 — 1마리 고정 (MVP). 하드코딩 타일 좌표.
		// 아레나 내부, Player(5,10)로부터 떨어진 지점. 충돌 맵에서 통행 가능 타일이어야 함.
		static constexpr int32 MonsterSpawnTileX = 20;
		static constexpr int32 MonsterSpawnTileY = 10;
		World->SpawnActor<AMonsterActor>(MonsterSpawnTileX, MonsterSpawnTileY);
	}
